...
class Learner:
    ...
    def __init__(self, opt: argparse.Namespace) -> None:
        ...
        self._metrics = get_metrics(self._problem_type)
        
    def fit(self, data: Tuple) -> None:
       models = get_models(self._model_types, self._problem_type)
    ...
    def _fit_holdout(self, models: List, data: Tuple) -> None:
            print('Fitting holdout...')
            X_train, X_test, y_train, y_test = data
            X_train, X_test, scaler = normalise_data(X_train, X_test, self._normalization)
            res = {}
            res['scaler'] = scaler
