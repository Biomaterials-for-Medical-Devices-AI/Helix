...
def save_actual_pred_plots(data, results, opt: argparse.Namespace, 
                            logger) -> None:
    ...
    if opt.problem_type == 'regression':
        ...
        directory = f'./log/{opt.experiment_name}/{opt.ml_log_dir}/plots/'
        ...
        for model_name, model_options in opt.model_types.items():
            if model_options["use"]:
                ...
                for i in range(opt.n_bootstraps):
                    ...
                    sns.scatterplot(x=y_test[i], y=y_pred_test, marker='o', s=30, color='black')
