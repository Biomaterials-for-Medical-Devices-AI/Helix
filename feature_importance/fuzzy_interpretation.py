...
def run(fuzzy_opt, ml_opt, data,  models, ensemble_results, logger):
    ...
    #gloabl_importance_results, local_importance_results, ensemble_results = interpreter.interpret(models, data)
    # ensemble_results = {'Majority Vote': pd.DataFrame()}
    # ensemble_data = {0: [1.000000, 0.035145, 0.169412, 0.221469, 0.037242, 0.125516, 0.025160,  0.198182, 0.023214, 0.056053,
    ...
    # ensemble_results['Majority Vote'] = pd.DataFrame(ensemble_data, index=index)

    test_df = fuzzy.interpret(models, ensemble_results, data, ml_opt)
    # print(test_df)
